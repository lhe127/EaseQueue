    ...
    <div class="bg-gray-200 min-h-screen select-none">
        <div class="fixed bg-white text-blue-800 px-10 py-1 z-10 w-full">
            <div class="flex items-center justify-between py-2 text-5x1">
                ...
                <div class="flex items-center text-gray-500">
                    <a class="relative material-icons-outlined p-2" style="font-size: 30px"
                        href="{{ route('adminMailbox') }}">
                        notifications
                    ...
                    <div onclick="toggledropdown()"
                        class="bg-center bg-cover bg-no-repeat rounded-full inline-block h-12 w-12 ml-2"
                        style="background-image: url(https://koululainen.fi/wp-content/uploads/2024/06/kapybara_verkkoon.jpg)">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="w-2/12 mr-6">
                <div class="bg-white rounded-xl shadow-lg mb-6 px-6 py-4">
                    ...
                    <div id="settingDropdown" class="hidden bg-white mt-2 rounded-md shadow-lg">
                        <a href="{{route('adminSetDepartment')}}"
                            class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-200">Department</a>
                        <a href="{{route('adminSetStaff')}}"
                            class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-200">Staff</a>
                        <a href="{{route('adminSetQueue')}}"
                            class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-200">Queue</a>
                    </div>
        ...
        <div class="rounded border-gray-500 bg-white absolute shadow-md top-[4.8rem] right-[1rem] hidden" id="dropdown">
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button type="submit"
                    class="flex justify-center items-center h-[3rem] w-[9rem] text-xl hover:bg-gray-200 cursor-pointer">
                    Log out
                </button>
            </form>
        </div>
